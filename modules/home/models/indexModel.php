...
function get_product_search($slug){
     $data = db_fetch_array("SELECT*FROM`products`WHERE`friendly_url`LIKE '%$slug%' limit 0,40 ");
    return $data;
}
function get_slider(){
    return db_fetch_array("SELECT*FROM`sliders` WHERE `status_id`=1 order by `location` asc");
}