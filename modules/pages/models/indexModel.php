...
function get_cat_id_1($slug)
{
    ...
    $item = db_fetch_array("SELECT*FROM`products`WHERE $query limit 0,8");
    return $item;
}
...
function get_product_sale(){
    return db_fetch_array("SELECT*FROM`products` order by `discount` desc limit 0,10");
}
function get_page($slug){
    return db_fetch_row("SELECT*FROM`pages`WHERE `friendly_url`='{$slug}'");
}
function get_post($slug){
    return db_fetch_row("SELECT*FROM`posts`WHERE `friendly_url`='{$slug}'");
}
function get_post_in_page($page_id){
    return db_fetch_array("SELECT*FROM`posts`WHERE `page_id`='{$page_id}' order by `created_at` desc");
}
